...
<head>
<meta charset="utf-8"/>
	<title><c:if test="${doType eq \"add\"}">添加</c:if><c:if test="${doType eq \"edit\"}">修改</c:if>TestCase</title>
						...
						<div class="portlet-body">
								  ...
								  <div class="portlet">
									<div class="portlet-title">
										<div class="caption">
											<i class="fa fa-tags"></i><c:if test="${doType==\"add\"}"> 添加</c:if><c:if test="${doType==\"edit\"}"> 修改</c:if>TestCase
										</div>
						  ...
						  <div class="portlet-body form">
									<form <c:if test="${doType eq \"add\"}">action="${ctx}/backendtestCase/saveTestCase"</c:if>  <c:if test="${doType eq \"edit\"}">action="${ctx}/backendtestCase/${testCase.id}/update"</c:if> method="post" id="form_testCase" class="form-horizontal">
										<div class="form-body">
											...
											<div class="alert alert-success display-hide">
										    	...
										    	<div class="form-group">
													...
													<div class="col-md-4">
														<input type="text" name="id" data-required="1" class="form-control" value="<c:out value='${testCase.id}'/>" />
													</div>
												</div>
										    	<div class="form-group">
													<label class="control-label col-md-1">用例名称
													<span class="required">
														 *
													</span>
													</label>
													<div class="col-md-4">
														<input type="text" name="name" data-required="1" class="form-control" value="<c:out value='${testCase.name}'/>" />
													</div>
												</div>
										    	<div class="form-group">
													<label class="control-label col-md-1">描述
													<span class="required">
														 *
													</span>
													</label>
													<div class="col-md-4">
														<input type="text" name="desc" data-required="1" class="form-control" value="<c:out value='${testCase.desc}'/>" />
													</div>
												</div>
											<!-- 图片上传的例子-->
											<!-- 
												...
												<div class="col-md-9">
													<div class="fileinput fileinput-new" data-provides="fileinput">
														<div class="fileinput-new thumbnail" style="width: 120px; height: 120px;">
															...
															<c:if test="${doType eq \"edit\"}"><img  class="thumb_img" src="${testCase.headimg}" alt=""/></c:if>
															<input id="headimg" type="text" name="headimg" class="hide" value="${testCase.headimg}" style="display: none; ">
														</div>
											...
											<div class="form-group">
												...
												<div class="col-md-8">
													<div id="div1">${testCase.content}</div>
													<textarea  style="display: none;" id="content" name="content"><c:out value='${testCase.content}'/></textarea>
												</div>
												<div class="clearfix margin-top-10">
														<span class="label label-danger">
															 备注
...
<script src="${ctx}/assets/scripts/backend/testCase/form-testCase-validation.js" ></script>
<!-- 上传文件 -->
...
jQuery(document).ready(function() {   
	...
	$("#testCaseManageMenu").addClass("active");
	$("#menu_1").text("业务管理");
	$("#menu_2").text("TestCase管理");
	
	   $('#cancelBtn').on( 'click', function () {
	       window.location.href = "${ctx}/backendtestCase/testCaseManagement?activeTab=tab_0";
	    } );
